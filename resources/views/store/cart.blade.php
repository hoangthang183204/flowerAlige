...
@section('content')
    ...
    @else
        <form action="{{ route('cart.update') }}" method="POST">
                        ...
                        @foreach ($items as $item)
                            <tr style="border-top:1px solid #f0f0ec;">
                                ...
                                <td style="padding:.7rem .9rem;text-align:center;">
                                    <input type="number" class="cart-qty" data-product-id="{{ $item['product']->id }}"
                                        data-price="{{ $item['product']->price }}" data-stock='@json($item['product']->stock)'
                                        name="items[{{ $item['product']->id }}][quantity]" value="{{ $item['quantity'] }}"
                                        min="0"
            ...
            <div
                style="display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:.75rem;margin-bottom:1.25rem;">
        ...
        (function() {
            ...
            function recalc() {
                ...
                qtyInputs.forEach(function(inp) {
                    ...
                    const stock = (stockRaw === undefined || stockRaw === '' || stockRaw === 'null') ? null :
                    ...
                    const msgEl = inp.parentElement.querySelector('.cart-stock-msg');
