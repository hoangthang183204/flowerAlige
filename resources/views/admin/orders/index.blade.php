                        ...
                        @forelse($orders as $order)
                            <tr>
                                ...
                                <td style="text-align:center;">
                                    @php
                                        $statusMap = [
                                            'pending' => ['label' => 'Chờ xử lý', 'badge' => 'admin-badge-muted'],
                                            'confirmed' => ['label' => 'Đã xác nhận', 'badge' => 'admin-badge-success'],
                                            'shipping' => ['label' => 'Đang giao', 'badge' => 'admin-badge-warning'],
                                ...
                                <td style="text-align:right;" class="font-semibold admin-accent">
                                    {{ number_format($order->total_amount, 0, ',', '.') }} đ</td>
                                <td style="text-align:right;" class="text-xs text-gray-500">
                                    {{ $order->created_at->format('d/m/Y H:i') }}</td>
                                <td style="text-align:right;">
                                    <a href="{{ route('admin.orders.show', $order) }}"
                                        class="admin-btn admin-btn-sm admin-btn-accent">Xem</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" style="text-align:center; padding: 2rem 1rem;"
                                    class="text-sm text-gray-500">Chưa có đơn hàng nào.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
